...
import qualified Graphics.Gnuplot.Plot.ThreeDimensional
                                               as P3d
import           Graphics.Gnuplot.Frame.Option  ( custom
                                                , pm3d
                                                )
...
import qualified Graphics.Gnuplot.LineSpecification
                                               as LineSpec
...
plotTermFrame term' frame =
  ...
  where (str, _ts) = fileContents "./" term' frame

plotPairs fpath xlabel ylabel title xss = plotTermFrame
  (PNG.cons fpath)
  (Frame.cons
...
plotPairsLabels fpath xlabel ylabel title labxss = plotTermFrame
  (PNG.cons fpath)
  (Frame.cons
    (OSet.title title . OSet.xLabel xlabel . OSet.yLabel ylabel $ OSet.deflt)
    (foldMap
...
plotPairsPoints fpath xlabel ylabel title xss = plotTermFrame
  (PNG.cons fpath)
  (Frame.cons
    (OSet.title title . OSet.xLabel xlabel . OSet.yLabel ylabel $ OSet.deflt)
    ( fmap (G2d.lineSpec (globalLineSpec . LineSpec.title "" $ LineSpec.deflt))
...
plotErrorBars fpath xlabel ylabel title xs = plotTermFrame
  (PNG.cons fpath)
  (Frame.cons
    (OSet.title title . OSet.xLabel xlabel . OSet.yLabel ylabel $ OSet.deflt)
    ( fmap (G2d.lineSpec (globalLineSpec . LineSpec.title "" $ LineSpec.deflt))
...
plot3d fpath xlabel ylabel title xs ys xyToZ = plotTermFrame
  (PNG.cons fpath)
  (Frame.cons
    ( OSet.key False
    . OSet.title title
