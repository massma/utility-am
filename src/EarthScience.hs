...
import qualified Numeric.Units.Dimensional.SIUnits
                                               as SI
import qualified Numeric.Units.Dimensional.NonSI
                                               as NonSI
...
sumCosSinSeries an bn theta =
  L.sum $ (\(a, b, n) -> a * cos (n * theta) + b * sin (n * theta)) <$> L.zip3
    an
    bn
...
h0
  ...
  -> Double -- ^ h0 (radians)
h0 lat dec | x > 1.0    = 0.0
           | x < (-1.0) = pi
           | otherwise  = acos x
  where x = -tan lat * tan dec
...
distanceFromDegreeLatLon
  :: (Double, Double) -- ^ first lat, lon
  -> (Double, Double) -- ^ second lat, lon
  -> Double -- ^ km
...
vaporSatFromT
  ...
  -> Double -- ^ saturation vapor pressure in Pa
vaporSatFromT t = 610.8 * exp (17.27 * t / (237.3 + t))

-- | equation 2.19 in Shuttleworth
rhFromTDew
  :: Double -- ^ temperature (celsius)
  -> Double -- ^ dew point temperature (celsius)
  -> Double -- ^ rh in percent
