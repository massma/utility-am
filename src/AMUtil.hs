...
utcTimeConstructor year month day hour minute sec = DT.UTCTime
...
averageVariance
  ...
  => t1 t2
  -> Maybe (t2, t2)
averageVariance xs = average xs >>= \xbar ->
...
utcToLocal :: Double -> Int -> Double
utcToLocal lon t | t' < 0    = t' + 24
                 | t' >= 24  = t' - 24
                 | otherwise = t'
  where t' = (fromIntegral t) + hourOffset lon

localToUtc :: Double -> Double -> Int
localToUtc lon t | t' < 0    = t' + 24
                 | t' >= 24  = t' - 24
                 | otherwise = t'
  where t' = round (t - hourOffset lon)
